...
def model_inference(args):
    ...
    for task in args.tasks:
        ...
        for cur_pred, entry in zip(cur_preds, data):
            ...
            # print(f"before extract:{pred}")
            # print("=========================")
            # pred = extract_block(pred)
            # print(f"after extract:{pred}")
            if args.instruct_mode:
                all_preds.append({
                    "task_id": entry["metadata"]["task_id"],
                    "pred": extract_block(cur_pred.outputs[0].text.split("<|fim_pad|>")[0].split("<|file_sep|>")[0]),
                    "origin_pred": cur_pred.outputs[0].text,
                    "origin_pred_len": len(tokenizer.encode(cur_pred.outputs[0].text)),
                    "task_type": task,
