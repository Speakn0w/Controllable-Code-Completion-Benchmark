...
def compute_metric_stmt(args):
    ...
    avg_em = sum(res["em"] for res in all_task_results.values()) / num_tasks
    avg_es = sum(res["es"] for res in all_task_results.values()) / num_tasks  
    avg_es_repoeval = sum(res["es_repoeval"] for res in all_task_results.values()) / num_tasks
    ...
    merged_results = {
        "overall": {
            "em": round(avg_em, 4),
            "es": round(avg_es, 4), 
            "es_repoeval": round(avg_es_repoeval, 4),
            "total": total_samples
        },
        "per_task": all_task_results
    }
    ...
    print("\nOverall Results (Unweighted Average):")
    print(f"EM: {avg_em:.2f}")
    print(f"ES: {avg_es:.2f}") 
    print(f"ES RepoEval: {avg_es_repoeval:.2f}")
    print(f"Total Samples: {total_samples}")
